...
if (!isset($_SESSION['active'])) {
  ...
  $where = "";
//-----------------------------------------------------------SABER SI ESTA EN DOS GRUPOS----------------------------------------------------
   $consultarGRUPO="SELECT a.ID_USU_ENC ,b.NOMBRE_USU from SEG_PLOMEROSPORENCARGADO a
left join SEG_USUARIOS b on a.ID_USU_ENC = b.ID_USU
where a.PLO_ID=&id_ and a.ID_USU_ENC in(380,381)";
  $resultadogrupo= sqlsrv_query($con, $consultarGRUPO);
if (sqlsrv_fetch_array($resultadogrupo)>0) {
   $grupos=2;
}else{
   $grupos=1;
}
...
<body>
  ...
  <!--  ---------------------------------------------GRUPOS------------------------------------>
    <div class="col row  col-md-3">
        <select class="form-control mt-1" id="select-Grupo" >
      <option value="">Seleccione un Grupo</option>
         <option value="380">Grupo 1 </option>
         <option value="381">Grupo 2</option>
    </select>
    </div>
  <div class="col row  col-md-6" style="width:600px">
   ...
   <div class="">
    ...
    <div class=""> 
      ...
      <select class="form-control selectpicker" id="sec" style="display: none">
      </select>
    ...
    <tbody>
      ...
      while ($row = sqlsrv_fetch_array($resultado)) {?>
        <tr>
          ...
          <td><a href="Resultado.php?sol_ID=<?php echo $row['sol_id']; ?>&%20id_=<?php echo $id_; ?>&%20lote=<?php echo $row['lote']; ?>&%20Servicios=<?php echo $row['servicio']; ?>"><span class="codigo">Asignar</span></a></td>
        </tr>
      <?php }?>
    </tbody>
  </table>
...
$('#select-Grupo').on('change',function(){
    ...
    var category=$("#select-Grupo").val();
    var url="select_category.php";
    var url2="select_category2.php";
